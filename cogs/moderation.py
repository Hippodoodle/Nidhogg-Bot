...
class Moderation(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if to_be_modded:
            ...
            """ Moderation log message handling """
            if log_channel != None:
                embed3 = discord.Embed(title="Potential Problematic Word Flagged", timestamp=datetime.datetime.utcnow(), color=0xed900c)
                embed3.add_field(name="User:", value=message.author, inline=True)
                embed3.add_field(name="User id:", value=message.author.id, inline=True)
                embed3.add_field(name="Nickname:", value=message.author.nick, inline=True)
                embed3.add_field(name="Channel:", value=message.channel, inline=True)
                embed3.add_field(name="Message:", value=message.content, inline=True)
                await log_channel.send("<@&721848925887397890>", embed=embed3)
